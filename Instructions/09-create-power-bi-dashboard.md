...
1. 開啟 Power BI Desktop。
    ...
    *根據預設，[用戶入門] 對話方塊會在Power BI Desktop 前面開啟。登入，然後關閉快顯。*
...
1. 注意功能區下方的黃色警告訊息。 *此訊息會提醒您查詢尚未套用為模型數據表載入的事實。您稍後會在實驗室中套用查詢。*
...
在這項工作中，您將藉由建立數據集來設定實驗室的環境。 *如果您已經發佈數據集，請移至下一個工作。*
...
*您將在下一個練習中重新整理數據集。視您的數據和報表而定，您可以隨時執行臨機操作數據重新整理或設定排程。不過，排程的重新整理需要無法為此實驗室設定的閘道。因此，從 Power BI Desktop，您將執行手動數據重新整理，然後將檔案上傳至您的工作區。*
