...
*注意：雖然啟用這兩個選項在開發數據模型時很有説明，但您稍早停用它們以支援實驗室體驗。當您在 Power BI Desktop** 實驗室的**載入轉換數據中建立關聯性時，您將了解為何要新增每個關聯性。*
...
您現在已將數據匯入 Power BI，並已開啟 Power Query 編輯器 以供下一個工作使用。
...
1. 將游標停在 [Ware House]**** 列上方，並注意有五個具有此值的資料列。
    ...
    *您將套用轉換，以在Power BI Desktop**實驗室中**重新標記這五個數據列。*
...
1. 檢閱 **TotalProductCost** 資料行的資料行品質，並注意有 8% 的資料列為空白。
    ...
    *遺漏 **TotalProductCost** 資料行值是數據質量問題。若要解決此問題，請在Power BI Desktop**實驗室的 **[載入轉換數據] 實驗室中，使用儲存在相關 **DimProduct** 數據表中的產品標準成本，套用轉換以填入遺漏的值。*
...
1. 在 [ **開啟** ] 視窗中，流覽至 **[D：\Allfiles\Resources** ] 資料夾，然後選取 **ResellerSalesTargets.csv** 檔案。 選取**開啟**。
...
1. 重複步驟，根據 **D：\Allfiles\Resources\ColorFormats.csv** 檔案建立查詢。
