...
*注意：雖然啟用這兩個選項在開發資料模型時很有説明，但您稍早停用它們以支援實驗室體驗。當您在 Power BI Desktop ** 實驗室的載入資料中 ** 建立關聯性時，您將瞭解為何要新增每個關聯性。*
...
1. 在資料預覽窗格中，水平捲動以檢閱所有資料行。 請注意，最後五個資料行包含**資料表**或**值**連結。
    ...
    *這五個數據行代表資料庫中其他資料表的關聯性。它們可用來將資料表聯結在一起。您將在 Power BI Desktop ** 實驗室的 ** 載入資料中聯結資料表。*
...
1. 水平捲動以顯示後面的資料行。 請注意 **DimProductSubcategory** 資料行。
    ...
    *當您在 Power BI Desktop ** 實驗室中 ** 將轉換新增至此查詢時，您將使用 ** DimProductSubcategory 資料行 ** 來聯結資料表。*
...
1. 將游標停在 [Ware House]**** 列上方，並注意有五個具有此值的資料列。
    ...
    *您將套用轉換，以在 Power BI Desktop ** 實驗室的 ** 載入資料中重新標記這五個數據列。*
...
1. 檢閱 **TotalProductCost** 資料行的資料行品質，並注意有 8% 的資料列為空白。
    ...
    *遺漏 ** TotalProductCost ** 資料行值是資料品質問題。若要解決此問題，請在 Power BI Desktop ** 實驗室的 ** [載入資料] 實驗室中，使用儲存在相關 ** DimProduct ** 資料表中的產品標準成本，套用轉換以填入遺漏的值。*
