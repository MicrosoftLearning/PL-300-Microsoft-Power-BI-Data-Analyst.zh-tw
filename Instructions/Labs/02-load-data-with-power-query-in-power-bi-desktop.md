...
1. 選取 **DimReseller** 查詢，並將 重新命名為 **Reseller**。
...
1. 移除所有資料行， **但下列專案除外** ：
    ...
    - DimProduct
        ...
        *注意：您可能會回想Power BI Desktop實驗室**中的準備資料**，其中一小部分**FactResellerSales 資料**列遺漏**TotalProductCost**值。**DimProduct**資料行已包含以擷取產品標準成本資料行，以協助修正遺漏的值。*

1. 展開 [DimProduct] 資料行、取消選取所有資料行，然後僅包括 [StandardCost] 資料行。

1. 若要建立自訂資料行，請在 [ **新增** 資料行] 功能區索引標籤的 [ **一般** ] 群組內選取 [ **自訂資料行**]。
...
1. 在 [自訂資料行公式] 方塊中，(在等號後面) 輸入下列運算式：
    ...
    - *此運算式會測試 **TotalProductCost** 值是否遺失。如果遺失，它會藉由將 **OrderQuantity** 值乘以 **StandardCost** 值來產生值;否則，它會使用現有的 **TotalProductCost** 值。*
...
1. 將下列三個資料行資料類型修改為 [固定的小數位數]。
    ...
    *固定的十進位數資料類型允許 19 位數，並允許更精確以避免四捨五入錯誤。請務必針對財務值使用固定的十進位數類型，或 (例如匯率) 。*
...
1. 選取 **ResellerSalesTargets** 查詢，並將重新命名為 **[目標**]。
...
1. 在 [**取代值**] 視窗的 [**要尋找的值**] 方塊中，輸入**M**並將 **[以空白取代]。**
