...
*此方法會建立報表和語意模型。我們只會使用此語意模型在此練習中建立新的報表。同一個程式可以使用來自不同報表的現有語意模型來完成，而不是上傳新的。此外，如果您未使用報表，工作區最佳做法會建議您刪除不必要的檔案。*

## **建立報表**

在這項工作中，您將建立與上一個工作中建立之 Power BI 語意模型的即時連線，然後建立新的 **銷售探索** 報表。
...
1. 在 [首頁] 功能區中，選取 **Power BI 語意模型**> OneLake 數據中樞。

1. 在 OneLake 數據中**樞**視窗中，選取 **[我的工作區 **] 中的 **[銷售分析**] 語意模型，然後按兩下 **[連接**] 或按兩下以載入語意模型。
