...
- 建立和編輯視覺計算。
- 使用 PREVIOUS（）、RUNNINGSUM（） 和 MOVINGAVERAGE（） 函式，在每個會計年度之間建立比較計量。
...
將資料夾解壓縮到 **C：\Users\Student\Downloads\07-visual-calculations** 資料夾。
...
1. 在 [ **視覺效果]** 窗格中，選取叢集條形圖視覺效果類型。
   ...
   ![圖片 01](Linked_image_Files/07-create-visual-calculations_image01.png)
...
1. 使用三點功能表，依年份**遞增排序產生的條形圖**，然後選取 [年份 **]，**然後選取 **[排序遞增**]：
   ...
   ![圖片 02](Linked_image_Files/07-create-visual-calculations_image02.png)
...
1. 選取條形圖後，選取功能區中的 [ **新增視覺效果計算** ]：
   ...
   ![圖片 03](Linked_image_Files/07-create-visual-calculations_image03.png)
...
1. 確認您現在會在畫面底部的視覺矩陣上看到 Profit 資料行：
   ...
   ![圖片 04](Linked_image_Files/07-create-visual-calculations_image04.png)

1. 展開 [新增視覺效果計算 **] 底下的**功能表，然後從範本選項中選取 **[與先前**的比較]：

    > **與上** 一個比較值與前一個值，因此我們看到 [收益] 與去年的上一個值比較。

   ![圖片 05](Linked_image_Files/07-create-visual-calculations_image05.png)
...
1. 從範本功能表中選取 [**移動平均**]，並將佔位元取代為 ，並將`WindowSize`佔位元`[Profit]`取代`[Field]`為 2。 您現在應該已設定下列專案：
   ...
   ![圖片 06](Linked_image_Files/07-create-visual-calculations_image06.png)
...
1. 在 **X 軸** 井/區域底下，選取下列欄位的可見性圖示，以從視覺效果中隱藏它們：
   ...
   ![圖片 07](Linked_image_Files/07-create-visual-calculations_image07.png)
...
1. 確認視覺效果現在符合目標。 結束視覺效果計算編輯畫面到報表：
   ...
   ![圖片 08](Linked_image_Files/07-create-visual-calculations_image08.png)
...
1. 將 `[Field]` 佔位元取代為 `[Sum of Sales]` ，並認可變更。 視覺效果看起來應該如下所示：
   ...
   ![圖片 09](Linked_image_Files/07-create-visual-calculations_image09.png)
