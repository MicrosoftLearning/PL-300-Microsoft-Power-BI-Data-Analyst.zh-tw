...
在此實驗室中，您將會建立一份三頁報表。 接著，您會將其發佈至 Power BI，您將在其中開啟報表並與其互動。
...
1. 在**D：\PL300\Labs\06-design-report-in-power-bi-desktop\Starter**資料夾中開啟**銷售分析**檔案。

1. **移至**  >  [檔案**另存新**檔]，然後將複本儲存在**D：\PL300\MySolution**資料夾中，以建立檔案的複本。
...
1. 在Power BI Desktop中，若要重新命名頁面，請在左下方以滑鼠右鍵按一下**第 1 頁**，然後將頁面**重新命名**為 [**概觀**]。
...
1. 若要新增交叉分析篩選器，請先按一下報表頁面的空白區域來取消選取影像，然後在 [**視覺效果**] 窗格中選取交叉分析**篩選器**。

     ![圖片 49](Linked_image_Files/07-design-report-in-power-bi-desktop_image18.png)

1. 在 [ **資料** ] 窗格中，將 [ **日期 \| 年份** ] 欄位 (不是階層的 **[年** ] 層級，) 拖曳到 [視覺效果] 窗格中的交叉分析篩選器 **欄位** 。
    
    *實驗室使用速記標記法來參考欄位。其看起來將會像下面這樣：[Date \| Year]。在此範例中，**Date** 是資料表名稱，而 **Year** 則是欄位名稱。*

1. 若要將交叉分析篩選器從清單轉換成下拉式清單，請流覽至 [視覺效果] > [將視覺效果格式化] **> Visual >交叉分析篩選器設定>樣式**]，然後從下拉式功能表中選取 [ **下拉** 式清單]。
...
1. 在視覺效果欄位窗格中 (位於 [ **視覺效果** ] 窗格下方) ，請注意，欄位會指派給 **X 軸** 和 **資料行 y 軸** 的 wells/areas。
    
    *藉由將欄位拖曳至視覺效果，這些欄位將會新增至預設的欄位/區域。為了有效位數，您可以直接將欄位拖曳到 wells/areas，因為接下來要這麼做。*

     ![圖 27](Linked_image_Files/07-design-report-in-power-bi-desktop_image28_N.png)

1. 從 [ **資料]** 窗格中，將 **[銷售 \| 利潤]** 欄位拖曳到 **[線條 y 軸** ] 的 [寬度/區域]。
...
1. 在視覺欄位窗格中，于 **[X 軸** ] 的 [寬度/區域] 中，針對 [ **月份** ] 欄位選取向下箭號，然後選取 [ **顯示沒有資料的專案**]。
    
    *請注意， **2020 年 6 月** 現在會出現。*
...
1. 從 [ **資料** ] 窗格中，將 [ **產品 \| 類別]** 欄位拖曳到 **[此頁面上的篩選** ] 區/區域。
...
1. 格式化視覺效果：
     ...
     - 在 [ **一般>效果] > [背景** ] 群組中，將 **[色彩** ] 設定為淺灰色 (，例如 「白色，20% 較深) ，以提供對比
 ...
 *現在可以看到以兩種不同視覺效果類型表示的相同資料。不過，這不是頁面版面配置的良好用法，不過，您可以藉由迭加視覺效果，在 Power BI Desktop 實驗室**的增強報表中**加以改善。藉由將按鈕新增至頁面，您將允許報表使用者判斷兩個視覺效果中的哪一個可見。*

 *現在，第三個 (且是最後一個) 頁面的設計已完成。*

## **發佈報表**

在這項工作中，您會將報表發佈至Power BI 服務。
...
1. 請注意，在 [發佈至 Power BI] 視窗中已選取 [我的工作區]。
    ...
    *我們不會在此實驗室Power BI 服務中深入瞭解不同的專案。*
...
1. 檢閱工作區的內容。 請注意 [所有]、[內容] 和 [資料集] + 資料流程的流覽選項。
    1. *工作區中有四種類型的專案可以存在，我們將討論 **報表** 和 **資料集**。*
    1. *如果資料集看不到，您可能需要重新整理您的 Microsoft Edge 瀏覽器。*
