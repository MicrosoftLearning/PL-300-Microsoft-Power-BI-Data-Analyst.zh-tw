...
1. 在 [ ** 開啟] ** 視窗中，流覽至 ** D：\PL300\Labs\05-create-dax-calculations-in-power-bi-desktop-advanced\Starter **  資料夾，然後開啟 ** [銷售分析 ** ] 檔案。
...
1. 若要設定矩陣視覺效果欄位，請從 [ ** 資料 ** ] 窗格拖曳 ** [區域 \| 區域 ** ] 階層，然後將它放在視覺效果內。
    ...
    *實驗室會使用速記標記法來參考欄位或階層。看起來會像這樣： ** 區域 \| 區域 ** 。在此範例中， ** Region 是資料表名稱，而 ** Regions ** ** 是階層名稱。*
...
1. 確認矩陣視覺效果有四個資料行標頭。
    ...
    *在 Adventure Works 中，銷售領域會組織成群組、國家/地區和區域。除了美國以外的所有國家/地區，只有一個區域，以國家/地區命名。由於美國如此龐大的銷售領域，它分為五個銷售領域。*
...
1. 根據下列運算式，將量值新增至 [Sales]**** 資料表：
    ...
    *CALCULATE（） 函式是一個功能強大的函式，可用來操作篩選內容。第一個引數會採用運算式或量值（量值只是具名運算式）。後續引數允許修改篩選內容。*

    *REMOVEFILTERS（） 函式會移除使用中的篩選。它不可以接受任何引數，或資料表、資料行或多個資料行作為其引數。*
...
1. 在 [ ** 資料 ** ] 窗格中，確定 ** 已選取 [銷售所有區域 ** ] 量值（選取時，它會有深灰色背景），然後在公式列中，以下列公式取代量值名稱和公式：

    *提示：若要取代現有的公式，請先複製程式碼片段。然後，在公式列內選取 ，然後按 ** Ctrl+A ** 以選取所有文字。然後，按 ** Ctrl+V ** 貼上程式碼片段以覆寫選取的文字。 **然後按 Enter ** 鍵。*
    ...
    *量值已重新命名，以正確反映更新的公式。DIVIDE（） 函式會將 ** Sales 量值（未由篩選內容修改）除以 ** 已修改內容中的 Sales ** ** 量值，這會移除套用至 ** Region ** 資料表的任何篩選。*
...
1. 請注意，[國家/地區的銷售百分比]**** 量值公式與 [所有區域的銷售百分比]**** 量值公式略有不同。
    ...
    *差異在於分母藉由移除 Region 資料表之 ** Region ** ** 資料行上的篩選，而非 Region ** ** 資料表的所有 ** 資料行，來修改篩選內容。這表示會保留套用至群組或國家/地區資料行的任何篩選。其會達成以國家/地區百分比表示銷售的結果。*

1. 將 [國家/地區的銷售百分比]**** 量值新增至矩陣視覺效果。

1. 請注意，只有美國的區域會產生不是 100% 的值。
    
    *您可能還記得只有美國有多個區域。所有其他國家/地區都組成單一區域，這解釋了為何全部為 100%。*
...
1. 若要在視覺效果中改善此量值的可讀性，請使用此改良的公式來覆寫 [國家/地區的銷售百分比]**** 量值。
    ...
    *IF（） 函式會使用 ISINSCOPE（） 函式來測試區域資料行是否為層級階層中的層級。若為 true，則會評估 DIVIDE（） 函式。若為 false，則會傳回空白值，因為區域資料行不在範圍內。*
...
*新增至 Sales ** 資料表的 ** 量值已修改篩選內容，以達成階層式導覽。請注意，若要達到小計計算的模式，需要從篩選內容中移除某些資料行，而且必須移除所有資料行總計。*
...
2. 根據下列運算式將量值新增至 [銷售]**** 資料表，並格式化為零個小數位數：
    ...
    *TOTALYTD（） 函式會評估特定日期資料行的 ** Sales ** 資料行總和，在此案例中為運算式。日期資料行必須屬於標示為日期資料表的日期資料表，如在 ** Power BI Desktop ** 實驗室中建立 DAX 計算所完成。*

    *函式也可以採用第三個選擇性引數，代表一年的最後一個日期。沒有這個日期表示 12 月 31 日是年份的最後一個日期。對於 Adventure Works，6 月在他們一年中的最後一個月，因此會使用 「6-30」。*
...
4. 請注意年度內銷售值的累積。
    ...
    *TOTALYTD（） 函式會執行篩選操作，特別是時間篩選操作。例如，若要計算 2017 年 9 月的 YTD 銷售額（會計年度的第三個月），Date ** 資料表上 ** 的所有篩選都會移除，並取代為從年初開始的新日期篩選（2017 年 7 月 1 日），並延伸至內容日期期間的最後一個日期（9 月 30 日， 2017).*
...
1. 根據下列運算式，將另一個量值新增至 ** Sales ** 資料表：
    ...
    *變數會以唯一的名稱宣告，然後必須在 RETURN ** 關鍵字之後 ** 輸出量值運算式。與其他編碼語言變數不同的是，DAX 變數只能在單一公式中使用。*

    ***SalesPriorYear ** 變數會指派運算式，以計算已修改內容中 Sales ** 資料行的總 ** 和，該運算式會使用 PARALLELPERIOD（） 函式，從篩選內容中的每個日期移回 12 個月。*
...
*您將根據 Power BI Desktop ** 實驗室中 ** 設計報表中的資料模型來建立報表。*
