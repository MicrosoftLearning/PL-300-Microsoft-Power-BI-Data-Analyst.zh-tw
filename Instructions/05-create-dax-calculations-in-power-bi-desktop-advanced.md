...
1. 若要設定矩陣視覺效果欄位，請從 [ **資料** ] 窗格拖曳 [ **區域區域 \| ** ] 階層，並將它放在視覺效果內。
...
1. 在 [ **資料** ] 窗格中，確定選取 [ **銷售所有區域** ] 量值時 (選取，它會有深灰色背景) ，然後在公式列中，以下列公式取代量值名稱和公式：

    *提示：若要取代現有的公式，請先複製程式碼片段。然後，選取公式列內部，然後按 **Ctrl+A** 以選取所有文字。然後，按 **Ctrl+V** 貼上程式碼片段以覆寫選取的文字。然後按 **Enter**。*
...
1. 在矩陣視覺效果中，請注意量值已重新命名，而且每個群組、國家/地區現在會出現不同的值。
...
1. 請注意，[國家/地區的銷售百分比] 量值公式與 [所有區域的銷售百分比] 量值公式略有不同。
    ...
    *差別在於分母藉由移除 Region **資料表之 Region** **資料行** 上的篩選來修改篩選內容，而不是 **Region** 資料表的所有資料行。這表示會保留套用至群組或國家/地區資料行的任何篩選。它會達到代表國家/地區百分比的銷售結果。*

1. 將 [國家/地區的銷售百分比] 量值新增至矩陣視覺效果。

1. 請注意，只有美國的區域會產生不是 100% 的值。
    
    *您可能會記得只有美國有多個區域。所有其他國家/地區都組成單一區域，說明為何全部都是 100%。*
...
在此練習中，您將建立銷售年度 (YTD) 量值和年度銷售額 (YoY) 成長量值。
...
2. 根據下列運算式將量值新增至 [銷售] 資料表，並格式化為零個小數位數：
    ...
    *TOTALYTD () 函式會評估指定日期資料行的**Sales**資料行總和，在此案例中為運算式。日期資料行必須屬於標示為日期資料表的日期資料表，如同在**Power BI Desktop 實驗室中建立 DAX 計算**中所完成。*
...
1. 根據下列運算式，將另一個量值新增至 **Sales** 資料表：
    ...
    ***SalesPriorYear**變數會指派運算式，以計算使用 PARALLELPERIOD () 函式之修改內容中**Sales**資料行的總和，以從篩選內容中的每個日期移回 12 個月。*
...
1. 請注意，[2018 Jul] 的 [Sales YoY Growth] 值為 [2017 Jul] 的 [Sales] 值。
    ...
    *現在已測試公式的「困難部分」，您可以使用計算成長結果的最終公式來覆寫量值。*
...
1. 確認 **2018 年 7 月**的 YoY 成長為 **392.83%**。
    ...
    *YoY 成長量值可識別在去年相同期間，幾乎 400% (或 4 倍) 的銷售量。*
...
*您將根據在 Power BI Desktop 實驗室中**設計報表**中的資料模型來建立報表。*
