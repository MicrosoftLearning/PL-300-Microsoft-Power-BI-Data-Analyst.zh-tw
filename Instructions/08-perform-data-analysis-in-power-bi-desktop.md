...
1. 使用您的組織 （或提供的） 認證完成登入程式。 若 Microsoft Edge 提示您保持登入，請選取 [是]****。
...
*此方法會建立報表和數據集。我們只會使用此數據集在此練習中建立新的報表。這個相同的程式可以使用不同報表的現有數據集來完成，而不是上傳新的數據集。此外，如果您未使用報表，工作區最佳做法會建議您刪除不必要的檔案。*

## **建立報表**

在這項工作中，您將建立與上一個工作中建立之 Power BI 數據集的即時連線，然後建立新的 **銷售探索** 報表。

1. 開啟 Power BI Desktop。

    ![Power BI Desktop 圖示](Linked_image_Files/02-load-data-with-power-query-in-power-bi-desktop_image1.png)

    *重要事項：如果您已經開啟Power BI Desktop（從先前的實驗室中），請關閉該實例。*

    *提示：根據預設，[用戶入門] 對話框會在 Power BI Desktop 前面開啟。您可以選擇登入，然後關閉快顯。*

1. 在 [首頁] 功能區中，選取 **[取得數據] > Power BI 數據集**。

1. 在 [數據中**樞**] 視窗中，選取 **[我的工作區 **] 中的 **[銷售分析**] 數據集，然後 **連線** 或按兩下以載入資料集。
...
1. 若要將檔案發佈至我的**工作區**，請在 [首頁 **] 功能區索引標籤的 **[共用 **] 群組內**，選取 **[發佈**]，然後選取 [選取**** 以發佈]。
