...
1. 使用組織 (或提供) 認證來完成登入程式。 如果 Microsoft Edge 提示您保持登入，請選取 [是]。
...
*這個方法會建立報表和資料集。我們只會使用此資料集在此練習中建立新的報表。這個相同的程式可以透過來自不同報表的現有資料集來完成，而不是上傳新的。此外，如果您未使用報表，工作區最佳做法會建議您刪除不必要的檔案。*
...
1. 在 [常用] 功能區中，選取 [ **取得資料] > Power BI 資料集**。

1. 在 [**資料中樞**] 視窗中，選取 **[我的工作區**] 中的 **[銷售分析**] 資料集 **，然後按一下或**按兩下以載入資料集。

1. 流覽至 [**檔案] > [儲存**]，並將檔案名儲存為**D：\PL300\MySolution**資料夾中的銷售**探索**。

*您現在將會建立兩個報表頁面，並在每個頁面上使用不同的視覺效果來分析及探索資料。*

## **練習 2：建立散佈圖**

在本練習中，您將建立能夠以動畫方式顯示的散佈圖。
...
1. 將下列欄位新增至視覺效果區域：
    ...
    *實驗室會使用速記標記法來參考欄位。看起來會像這樣： **轉銷商** **\|** **商務類型**。在此範例中， **Reseller** 是資料表名稱， **而商務類型** 是功能變數名稱。*
...
1. 在篩選卡片中，依兩年進行篩選：**FY2019** 和 **FY2020**。
    ...
    *在一段時間內預測時，您需要至少兩個週期 (年) 資料，才能產生精確的穩定預測。*
...
8. 展開 [預測] 區段。
    ...
    *如果 [ **預測** ] 區段無法使用，可能是因為視覺效果尚未正確設定。只有在符合兩個條件時才可使用預測：軸具有日期類型的單一欄位，而且只有一個值欄位。*
...
1. 若要將檔案發佈至 **[我的工作區**]，請在 [ **首頁** ] 功能區索引標籤的 **[共用** ] 群組內選取 [ **發佈** ]，然後選取 [ **選取** 發佈]。
