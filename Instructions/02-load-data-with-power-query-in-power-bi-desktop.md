---
lab:
  title: 載入轉換的資料Power BI Desktop
  module: 'Clean, Transform, and Load Data in Power BI'
---

# 載入轉換的資料Power BI Desktop
...
1. 在 [ **開啟** ] 視窗中，流覽至 **D：\PL300\Labs\02-load-data-with-power-query-in-power-bi-desktop\Starter** 資料夾，然後開啟 **Sales Analysis** 檔案。
...
在這項工作中，您將使用Power Query 編輯器來設定**Salesperson**查詢。
...
1. 若要重新命名查詢，請在 [查詢設定] (位於右側) 窗格的 [名稱] 方塊中，將文字更換為 **Salesperson**，然後按 **Enter**。 然後，確認名稱已在 **[查詢** ] 窗格中更新。
...
1. 在 [查詢設定] 窗格的 [套用的步驟] 清單中，請注意新增的**已篩選資料列**步驟。
    ...
    *您建立的每個轉換都會產生另一個步驟邏輯。可以編輯或刪除步驟。您也可以選取一個步驟，以在查詢轉換的該階段預覽查詢結果。*

     ![套用的步驟](Linked_image_Files/02-load-data-with-power-query-in-power-bi-desktop_image17.png)

1. 若要移除資料行，請在 [ **首頁** ] 功能區索引標籤上，選取 **[管理** 資料行] 群組，選取 [ **選擇資料行** ] 圖示。
...
1. 若要重新命名 **EmployeeNationalIDAlternateKey** 資料行，請按兩下 **EmployeeNationalIDAlternateKey** 資料行標頭，並以 **EmployeeID**取代文字，然後按 **Enter**。
...
*重要事項：當已提供詳細的指示時，實驗室步驟會提供更簡潔的指示。如果您需要詳細的指示，您可以回到先前工作的步驟。*

1. 選取 **DimProduct** 查詢，並將查詢重新命名為 **Product**。

1. 找出 **FinishedGoodsFlag** 資料行，然後篩選資料行以擷取已完成貨物 (，也就是 TRUE) 。
...
1. 若要重新命名新的資料行，請按兩下 **合併的資料** 行標頭，並將資料行重新命名為 **TargetMonth**。
...
1. 若要將 **[目標** 值] 乘以 1000，請選取 [ **目標** ] 資料行標頭，然後在 [ **轉換** ] 功能區索引標籤的 [ **數位資料行** ] 群組內，選取 [ **標準**]，然後選取 [ **乘法**]。

    *您可能還記得目標值是以千為單位儲存。*

     ![圖片 5682](Linked_image_Files/02-load-data-with-power-query-in-power-bi-desktop_image63.png)

1. 在 [ **相乘** ] 視窗的 [ **值** ] 方塊中，輸入 **1000**，然後選取 [ **確定**]。
...
1. 若要合併 **ColorFormats** 查詢，請在 [ **常用** ] 功能區索引標籤上，選取 **[合併** 向下箭號]，然後選取 [ **合併查詢**]。
...
1. 當 [ **隱私權等級** ] 視窗開啟時，針對這兩個數據源，在對應的下拉式清單中選取 [ **組織**]，然後選取 [ **儲存**]。

    *您可以設定資料來源的隱私權等級，以判斷是否可以在來源之間共用資料。將每個資料來源設定為 **[組織**] 可視需要共用資料。私人資料來源永遠無法與其他資料來源共用。這並不表示無法共用私人資料;這表示Power Query引擎無法在來源之間共用資料。*
...
1. 在 [查詢屬性] 視窗中，取消選取 [啟用載入至報告] 核取方塊。
    ...
    *停用載入表示不會載入為資料模型的資料表。這是因為查詢已與 **Product** 查詢合併，因此能夠載入至資料模型。*
