...
# 在 Power BI Desktop 中載入資料

**實驗室的完成時間估計為 45 分鐘。**

在此實驗室中，您會將轉換套用至在上一個實驗室中建立的每個查詢。 接著，您將會把每項查詢載入為資料模型的資料表。
...
1. 若要重新命名查詢，請在 [查詢設定] (位於右側) 窗格的 [名稱] 方塊中，將文字更換為 **Salesperson**，然後按 **Enter**。 然後確認名稱已在 **[查詢** ] 窗格中更新。
    
    *查詢名稱會決定模型資料表名稱。建議您定義簡潔且易記的名稱。*

1. 若要找出特定資料行，請在 [ **常用** ] 功能區索引標籤上，選取 [ **管理** 資料行] 向下箭號，選取 [ **選擇** 資料行] 向下箭號，然後選取 [ **移至資料行**]。
    
    *移至資料行是許多資料行的實用功能。否則，您可以水準捲動尋找資料行。*

     ![管理資料行>選擇>移至資料行的資料行](Linked_image_Files/02-load-data-with-power-query-in-power-bi-desktop_image13.png)
...
1. 選取 **SalesPersonFlag 資料** 行標頭，然後選取向下箭號，然後選取 **FALSE**，然後按一下 [ **確定**]。
    
    *此動作會篩選資料列，只擷取銷售人員的員工。*

1. 在 [查詢設定] 窗格的 [套用的步驟] 清單中，請注意新增的**已篩選資料列**步驟。
    
    *您在另一個步驟邏輯中建立的每個轉換都會產生結果。可以編輯或刪除步驟。您也可以選取一個步驟，在查詢轉換的該階段預覽查詢結果。*

     ![套用的步驟](Linked_image_Files/02-load-data-with-power-query-in-power-bi-desktop_image17.png)

1. 若要移除資料行，請在 [ **首頁** ] 功能區索引標籤上選取 [ **管理** 資料行] 群組，然後選取 [ **選擇資料行** ] 圖示。
...
*重要事項：提供詳細的指示後，實驗室步驟會提供更簡潔的指示。如果您需要詳細的指示，您可以回頭參閱先前工作的步驟。*

1. 選取 **DimProduct** 查詢，並將查詢重新命名為 **Product**。

1. 找出 **FinishedGoodsFlag** 資料行，然後篩選資料行，以擷取已完成商品 (，也就是 TRUE) 。
...
1. 在 **DimProductSubcategory** 資料行標頭中，選取資料行名稱右邊的展開按鈕。

    ![資料行展開圖示](Linked_image_Files/02-load-data-with-power-query-in-power-bi-desktop_image31.png)

1. 查看資料行的完整清單，然後選取 [ **選取所有** 資料行] 方塊以取消選取所有資料行。
2. 選取 **[EnglishProductSubcategoryName] 和 [DimProductCategory**]，然後取消核取 [**使用原始資料行名稱作為前置詞**] 核取方塊，然後選取 [**確定**]。** **
...
1. 展開 **DimGeography** 資料行， **只** 包含下列三個數據行：
...
1. 移除所有資料行， **但下列專案除外** ：
    ...
    - DimProduct
        ...
        *注意：您可能會在**Power BI Desktop實驗室的準備資料**中回想一下 **，一小部分的 FactResellerSales**資料列遺漏**TotalProductCost**值。**已包含 DimProduct**資料行來擷取產品標準成本資料行，以協助修正遺漏的值。*

1. 展開 [DimProduct] 資料行、取消選取所有資料行，然後僅包括 [StandardCost] 資料行。

1. 若要建立自訂資料行，請在 [ **新增資料行** ] 功能區索引標籤的 [ **一般** ] 群組內，選取 [ **自訂資料行**]。
...
1. 在 [自訂資料行公式] 方塊中，(在等號後面) 輸入下列運算式：
    ...
    - *此運算式會測試 **TotalProductCost** 值是否遺失。如果遺漏，它會將 **OrderQuantity** 值乘以 **StandardCost** 值來產生值;否則，它會使用現有的 **TotalProductCost** 值。*
...
1. 若要修改資料行資料類型，請在 **[數量]** 資料行標頭中，選取資料行名稱左邊的 **1.2** 圖示，然後選取 **[整數**]。
...
1. 將下列三個資料行資料類型修改為 [固定的小數位數]。
    ...
    *固定十進位數資料類型允許 19 位數，並允許更精確，以避免舍入錯誤。請務必針對財務值使用固定的十進位數類型，或 (例如匯率) 。*
...
1. 在 [ **取代值** ] 視窗的 [ **值至尋找** ] 方塊中，輸入 **M** ，並將 **[取代為** 空白]。

1. 將 [MonthNumber] 資料行的資料類型修改為 [整數]。

1. 在 [ **新增資料行]** 功能區索引標籤的 [ **一般** ] 群組內，選取 [ **來自範例的資料行** ] 圖示。
...
1. 在 [Column1] 資料行的第一個格線儲存格中，開始輸入 **7/1/2017**，然後按 **Enter**。
    ...
    *虛擬機器使用美國地區設定，因此此日期實際上是 2017 年 7 月 1 日。其他區域設定可能需要日期之前的 **0** 。*

1. 請注意，方格儲存格會使用預測的值更新。
    
    *此功能已正確預測您正在結合 **Year** 和 **MonthNumber** 資料行中的值。*

1. 另請注意查詢方格上方顯示的公式。

     ![圖片 5679](Linked_image_Files/02-load-data-with-power-query-in-power-bi-desktop_image60.png)

1. 若要重新命名新的資料行，請按兩下 **[合併** 的資料行] 標頭，並將資料行重新命名為 **TargetMonth**。
...
1. 若要將 **[目標** 值] 乘以 1000，請選取 [ **目標** ] 資料行標頭，然後在 [ **轉換** ] 功能區索引標籤的 [ **數位資料行** ] 群組內選取 [ **標準**]，然後選取 [ **乘積**]。
...
### **工作 9：設定 ColorFormats 查詢**

在這項工作中，您將設定 **ColorFormats** 查詢。

1. 選取 **ColorFormats** 查詢，並注意到第一個資料列包含資料行名稱。

1. 在 [ **首頁** ] 功能區索引標籤的 **[轉換** ] 群組內，選取 **[使用第一個資料列作為標頭**]。
...
1. 當 [ **隱私權等級** ] 視窗開啟時，針對這兩個數據源的每一個，在對應的下拉式清單中，選取 [ **組織**]，然後選取 [ **儲存**]。
    
    *您可以設定資料來源的隱私權等級，以判斷資料是否可以在來源之間共用。視需要將每個資料來源設定為**組織**，即可共用資料。私人資料來源永遠無法與其他資料來源共用。這並不表示無法共用私人資料;這表示Power Query引擎無法在來源之間共用資料。*

     ![圖片 5691](Linked_image_Files/02-load-data-with-power-query-in-power-bi-desktop_image74.png)

1. 在 [ **合併]** 視窗中，使用預設的 **[聯結種類** - 維護左方外部] 的選取範圍，然後選取 [ **確定**]。
...
1. 在 [查詢屬性] 視窗中，取消選取 [啟用載入至報告] 核取方塊。
    ...
    *停用載入表示它不會載入為資料模型的資料表。這是因為查詢已與 **Product** 查詢合併，因此能夠載入至資料模型。*
...
1. 在 [欄位] 窗格 (位於右側) 中，請注意載入資料模型的七份資料表。
