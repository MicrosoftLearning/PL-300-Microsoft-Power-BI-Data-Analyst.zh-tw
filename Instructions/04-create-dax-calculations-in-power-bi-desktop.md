...
在此實驗室中，您將使用資料分析運算式 (DAX) 來建立計算資料表、計算結果欄和簡單量值。
...
2. 在公式列 (在建立或編輯計算時直接在功能區下方開啟) 中，輸入 **Salesperson =**，按 **Shift+Enter**，輸入 **'Salesperson (Performance)'**，然後按 **Enter** 鍵。
    ...
    *為了方便起見，此實驗室中的所有 DAX 定義都可從程式碼片段檔案複製，位置為 **D:\PL300\Labs\04-create-dax-calculations-in-power-bi-desktop\Assets\Snippets.txt**。*
     ...
     *此資料表定義會建立 **Salesperson (Performance) 資料表的 ** 複本。它只會複製資料，不過模型屬性，例如可見度、格式等。不會複製。*

     *提示：建議您輸入「空白字元」 (，也就是歸位字元和索引標籤) 以直覺且容易閱讀的格式撰寫公式，特別是公式冗長且複雜時。若要輸入歸位字元，請按 **Shift+Enter**。「空白字元」是選擇性的。*

1. 請注意，在 [ **資料** ] 窗格中，資料表圖示是藍色 (表示計算資料表) 的陰影。

    ![圖片 10](Linked_image_Files/05-create-dax-calculations-in-power-bi-desktop_image11.png)

    *注意：匯出資料表是使用會傳回資料表的 DAX 公式來定義。請務必瞭解，匯出資料表會增加資料模型的大小，因為它們會具體化和儲存值。每當重新整理公式相依性時，就會重新計算這些資料模型，因為當新的 (未來) 日期值載入資料表時，就會重新計算這些資料模型的情況。*

    *不同於 Power Query 所產生的資料表，系統無法使用導出資料表來載入外部資料來源的資料。這些資料表只能根據已載入資料模型的資料來轉換資料。*

1. 切換至 [模型] 檢視，並請注意，您可能需要重設檢視以尋找資料表) ，才能使用 **Salesperson** (資料表。
...
1. 在 [屬性] 窗格的 [描述] 方塊中，輸入：**與銷售額相關的銷售人員**
    ...
    *您可能會記得，當使用者將游標停留在資料表或欄位上方時，描述會顯示為 **[資料** ] 窗格中的工具提示。*
...
1. 在公式列中輸入下列內容 (或從程式碼片段檔案複製)，然後按 **Enter**：
    ...
    *系統會先輸入資料行名稱，然後輸入等號 (=) ，後面接著會傳回單一值結果的 DAX 公式，來建立匯出資料行。資料表中還不能有資料行名稱。*
...
1. 切換回 [報表] 檢視。 在 [ **資料** ] 窗格中，確定選取 [ **月份]** 欄位 (選取時，其背景會) 深灰色。
...
1. 以滑鼠右鍵按一下 [重新命名]，將新建立的階層重新命名為 **[會計****]。**


1. 在 [**資料**] 窗格中選取 [會計階層]，以滑鼠右鍵按一下 [**新增至階層****] [會計**]，將下列兩個剩餘欄位新增至會計階層  ->  。
...
1. 儲存 Power BI Desktop 檔案。
    ...
    *Power BI Desktop現在瞭解此資料表定義日期 (時間) 。依賴時間智慧計算時非常重要。您將使用在 Power BI Desktop 實驗室中**建立進階 DAX 計算**中的時間智慧計算。*

    *當您在資料來源中沒有日期資料表時，適用于日期資料表的設計方法。如果您有資料倉儲，則適合從其日期維度資料表載入日期資料，而不是在資料模型中「重新定義」日期邏輯。*

## **建立簡單的量值**

在此工作中，您將建立簡單量值。 簡單量值會彙總資料表中單一資料行或計數資料列中的值。

1. 在 [報表] 檢視 **的第 2 頁**的 [ **資料** ] 窗格中，將 **[銷售 \| 單價** ] 欄位拖曳至矩陣視覺效果。
...
1. 選取 **Unit Price** 的向下箭號，並留意可用的功能表選項。
    ...
    *可見的數值資料行允許報表作者在報表設計階段決定資料行值如何摘要 (或未) 。這可能會導致不適當的報告。不過，有些資料模型工具不喜歡離開機會，並選擇隱藏這些資料行，並改為公開量值中定義的匯總邏輯。這是您現在將在此實驗室中採用的方法。*
...
14. 在 [報表] 檢視的 [矩陣視覺效果 **的值] 區** /區域中，針對 [ **單價** ] 欄位，選取 **[X** ] 將其移除。
...
在這項工作中，您將建立更多量值，以使用更複雜的公式。

1. 在 [報表] 檢視中，選取 **[第 1 頁** ] 並檢閱資料表視覺效果，並指出 **[目標** ] 資料行的總計。
...
1. 將 [目標 \| 目標] 資料行重新命名為 [目標 \| TargetAmount]。
    ...
    *提示：有數種方式可以在 [報表] 檢視中重新命名資料行：在 [ **資料** ] 窗格中，您可以以滑鼠右鍵按一下資料行，然後選取 [ **重新命名**]，或按兩下資料行，或按 **F2**。*
