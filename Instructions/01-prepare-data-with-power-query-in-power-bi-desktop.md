...
在這項工作中，您會從開啟入門 Power BI （.pbix） 檔案開始。 入門檔案不包含任何數據，但已特別設定來協助您完成實驗室。 入門檔案中已停用下列報表層級設定：

- 數據載入>第一次載入時從數據源匯入關聯性
- 數據載入>載入數據之後自動偵測新的關聯性

*注意：雖然啟用這兩個選項在開發數據模型時很有説明，但您稍早停用它們以支援實驗室體驗。當您在Power BI Desktop** 實驗室的載入資料中**建立關聯性時，您將了解為何要新增每個關聯性。*
...
1. 如果系統提示您輸入認證，請在 [**SQL Server 資料庫]** 視窗中選取 [**使用我目前的認證**]，然後 **連線**。

1. 在 [導覽器]**** 視窗中，展開左側的 **AdventureWorksDW2020** 資料庫。

    *注意：**AdventureWorksDW2020** 資料庫是以 AdventureWorksDW2017** 範例資料庫為基礎**。它已修改為支持課程實驗室的學習目標。*

1. 選取 -- 但未檢查 — **DimEmployee** 數據表
...
1. 在右窗格中，請注意資料表資料的預覽。 預覽數據可讓您查看資料列和數據列範例。
...
1. 按兩下 **[轉換數據**]，以完成這項工作，這會開啟 Power Query 編輯器。
    1. *此實驗室僅供連線及分析數據，但無法 **轉換數據**。*

## **預覽 Power Query 編輯器 中的數據**

此工作引進 Power Query 編輯器，並可讓您檢閱及分析數據。 這可協助您判斷稍後如何清除和轉換數據。
...
1. 在資料預覽窗格中，水平捲動以檢閱所有資料行。 請注意，最後五個資料行包含**資料表**或**值**連結。
    ...
    *這五個數據行代表資料庫中其他數據表的關聯性。它們可用來將數據表聯結在一起。您將在 Power BI Desktop** 實驗室的**載入數據中聯結資料表。*
...
1. 檢閱 EmployeeKey** 數據行的數據**行分佈—有 296 個相異值，以及 296 個唯一值。
    ...
    *當相異和唯一計數相同時，表示數據行包含唯一值。模型化時，某些模型數據表必須有唯一的數據行。這些唯一的數據行可用來建立一對多關聯性，您將在Power BI Desktop**實驗室的**模型數據中執行此作業。*
...
1. 在 [查詢]**** 窗格中，選取 **DimEmployeeSalesTerritory** 查詢。
    ...
    ***DimEmployeeSalesTerritory** 數據表會為每個員工和他們管理的銷售領域區域儲存一個數據列。此數據表支援將許多區域關聯至單一員工。有些員工管理一個、兩個或更多區域。當您建立此資料的模型時，您必須定義多對多關聯性。*

1. 在 [查詢]**** 窗格中，選取 **DimProduct** 查詢。 **DimProduct** 資料表會針對由公司銷售的每個產品，各包含一個資料列。

1. 水平捲動以顯示後面的資料行。 請注意 **DimProductSubcategory** 資料行。

    *當您在 Power BI Desktop** 實驗室中**將轉換新增至此查詢時，您將使用 **DimProductSubcategory 數據行**來聯結數據表。*
...
1. **選取 BusinessType** 數據行標頭，並注意數據預覽窗格下方的新窗格。
...
1. 將游標停在 [Ware House]**** 列上方，並注意有五個具有此值的資料列。
    ...
    *您將套用轉換，以在 Power BI Desktop** 實驗室的**載入數據中重新標記這五個數據列。*
...
1. 檢閱 **TotalProductCost** 資料行的資料行品質，並注意有 8% 的資料列為空白。
    ...
    *遺漏 **TotalProductCost** 資料行值是數據質量問題。若要解決此問題，請在Power BI Desktop**實驗室的 **[載入數據] 實驗室中，使用儲存在相關 **DimProduct** 數據表中的產品標準成本，套用轉換以填入遺漏的值。*
...
1. **儲存** Power BI Desktop 檔案。 當系統提示您套用暫止的變更時，請選取 [ **稍後**套用]。
    ...
    *提示：套用查詢會將其數據載入數據模型。您尚未準備好這麼做，因為必須先套用許多轉換。*
