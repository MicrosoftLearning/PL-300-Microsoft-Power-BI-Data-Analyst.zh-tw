...
在此練習中，您將建立八個Power BI Desktop查詢。 六個查詢將會從 SQL Server 取得來源資料，而兩個會從 CSV 檔案取得。

### **工作 1：開始使用 Power BI Desktop**

在這項工作中，您會從開啟入門 Power BI 檔案 (.pbix) 開始。 入門檔案不包含任何資料，但已特別設定來協助您完成實驗室。 入門檔案中已停用下列報告層級設定：
...
1. 重複步驟，根據 **D:\PL300\Resources\ColorFormats.csv** 檔案建立查詢。
    ...
    ***ColorFormats** CSV 檔案包含每個產品色彩一列。每個資料列都會記錄 HEX 程式碼，以格式化背景和字型色彩。*
...
1. **儲存**Power BI Desktop檔案。 當系統提示您套用擱置的變更時，請選取 [ **稍後套用**]。
    ...
    *提示：套用查詢會將資料載入至資料模型。您尚未準備好這麼做，因為必須先套用許多轉換。*
